...
using UnPack: @unpack
using PartiallyObservedInverseGames.TrajectoryVisualization: visualize_rotating_hyperplanes, display_3D_trajectory
...
function infer()
    ...
    # Load data 
    data_states   = Matrix(CSV.read("data/f_3d_s.csv", DataFrame, header = false))
    data_inputs   = Matrix(CSV.read("data/f_3d_c.csv", DataFrame, header = false))
    ...
    player_cost_models = map(enumerate(as)) do (ii, a)
        cost_model = CollisionAvoidanceGame.generate_3dintegrator_cost(;
            player_idx = ii,
            control_system,
            T,
            goal_position = scale*[cos(a), sin(a), zs[ii]],
            weights = (; 
                state_goal      = -1,
                control_Δv      = -1),
            T_activate_goalcost,
    ...
    (converged, solution_inverse) = solve_inverse_game(
            ...
            control_system,
            player_cost_models,
            init = (;s = 1.5, x = data_states),
            solver = Ipopt.Optimizer,
            solver_attributes = (; max_wall_time = 40.0, print_level = 5),
            cmin = 1e-5,
            ρmin,
            μ = μs[1],
        )
    ...
    #     vcat(solution_inverse.x[1:4, :], solution_inverse.x[7:10, :]), # dirt hack 
    #     (;
    #         ΔT,
    #         adjacency_matrix,
    ...
    #         title = "inferred_hyperplane_3d",
    #         n_players,
    #         n_states_per_player = 4, # dirt hack
...
function mc(solution_inverse, trials; rng = MersenneTwister(0), μ = 1.0, max_wall_time = 10.0, visualize = false)
    ...
    while i <= trials
        ...
        if norm(x0[1:2,:] - x0[7:8,:]) < 1.2*minimum(solution_inverse.ρs)
            continue
        end
        ...
        converged_forward, time_forward, solution_forward, _ = 
